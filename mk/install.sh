...
lvm_configuration_update_required()
{
  if test "${os_distro}" != rhel ; then
    return 1
  fi
  
  if test ${os_majorver} != 4 ; then
    return 1
  fi
  ...
  if [[ -z "$lvm_types" || "$lvm_types" != *\"xvd\"* ]] ; then
    return 0
  fi
...
modules_configuration_update_required()
{
...
{
  if test -z "`grep -e '^[ \t]*types' /etc/lvm/lvm.conf`" ; then
...
update_grub_configuration()
{
  ...
  # Ensure kernel initrd and module lines include /boot/ if it is not a separate filesystem.
  ...
  sed -e "s;^\(\([^\#]*\|\)kernel [^ ]*\) .*;\1 ${cmdline};g ; ${rewrite_path}" <"${conf}.bak" >"${conf}"
  ...
  if [ -L "/boot/xenkernel" ] && [ -x "/sbin/grubby" ] ; then
    ...
    /sbin/grubby --add-kernel=${kernel} $INITRD --copy-default --make-default --title="${k}"
  fi
...
update_vmlinuz_symlink()
{
  ...
  if [ -L "/boot/initrd.img" ] && [ "$(readlink -f /boot/initrd.img)" = "/boot/initrd.img-${version}" ] ; then
...
####################################
# Select Packages For Each Distro
##################################
...
{
  GUEST_PKG_TYPE=apk
  PKGNAME=$(apk info xe-guest-utilities 2>&1)
  ERROR=$?
  if [ $ERROR != 0 ] ; then
    XGU="xe-guest-utilities"
  else
...
select_pkgs_debian()
{
  ...
  case ${os_distro}:${os_majorver}:${os_minorver} in
    debian:[0-5]:*)
      os_support_failure "This version of Debian is no longer supported by these utilities."
      ;;
    debian:[6-9]:*)
      ;;
    debian:[1-9][0-9]:*)
    ...
    yinhe:*:*)
      ;;
    ubuntu:[1-9]:*)
      os_support_failure "This version of Ubuntu is no longer supported by these utilities."
      ;;
    ubuntu:[1-9][0-9]:*)
      ;;
    *)
      os_support_failure "Unknown Debian variant ${os_distro}:${os_majorver}:${os_minorver}"
      ;;
  esac
...
select_pkgs_sles()
{
  ...
  case "${os_distro}${os_majorver}" in
    ...
    *)
      os_support_failure "Unknown SuSE Linux variant \`${os_distro} ${os_majorver}'."
      ;;
  esac
...
install_alpine()
{
  echo "Installing xe-guest-utilities for Alpine Linux..."
...
case "${os_distro}" in
  rhel|centos|oracle|fedora)
    select_pkgs_rhel
    ;;
  scientific|neokylin|asianux|turbo)
    select_pkgs_rhel
    ;;
  cloudlinux|sangoma)
    select_pkgs_rhel
  ...
  *)
    os_support_failure "Unknown Linux distribution \`${os_distro}'."
    ;;
esac
...
if [ -n "${KERNEL}" -a -L "/boot/xenkernel" ] || [ -n "${KERNEL}" -a -L "/boot/vmlinuz" ] ; then
...
if modules_configuration_update_required ; then
...
if lvm_configuration_update_required ; then
...
if inittab_configuration_update_required ; then
...
if arp_notify_update_required ; then
